...
export const useGetProfileActivityListAction = () => {
  ...
  const fetchProfileActivityLists = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await getProfileActivityListApi(page);
  ...
  return {
    ...
    lists,
    error,
    refetch: fetchProfileActivityLists,
    handlePagination,
  };
};
