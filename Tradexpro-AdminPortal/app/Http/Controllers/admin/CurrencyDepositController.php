...
class CurrencyDepositController extends Controller
{
    ...
    public function currencyDepositPendingList(Request $request)
    {
        ...
        if ($request->ajax()) {
            $deposit_list = CurrencyDeposit::with('bank.bank_form')->where('status',STATUS_PENDING)->orderBy('currency_deposits.id', 'desc');
            ...
            return datatables()->of($deposit_list)
                ...
                ->addColumn('bank_receipt', function ($query) {
                    if(!empty($query->bank_receipt)) {
                        $action = modal_image_show($query->id, $query->bank_receipt, $query?->bank?->bank_form?->title ?? "Bank");
                        return $action;
                    } else {
                        return 'N/A';
                    }
    ...
    public function currencyDepositAcceptList(Request $request)
    {
        ...
        if ($request->ajax()) {
            $deposit_list = CurrencyDeposit::with('bank.bank_form')->where('status',STATUS_ACCEPTED)->select('currency_deposits.id',
    ...
    public function currencyDepositRejectList(Request $request)
    {
        ...
        if ($request->ajax()) {
            $deposit_list = CurrencyDeposit::with('bank.bank_form')->where('status',STATUS_REJECTED)->select('currency_deposits.id',
                                'currency_deposits.unique_code'
                                , 'currency_deposits.user_id'
                                , 'currency_deposits.wallet_id'
                                , 'currency_deposits.payment_method_id'
