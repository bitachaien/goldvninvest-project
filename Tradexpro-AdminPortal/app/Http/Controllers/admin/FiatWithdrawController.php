...
class FiatWithdrawController extends Controller
{
    ...
    public function fiatWithdrawPendingList(Request $request)
    {
        ...
        if ($request->ajax()) {
            $withdrawal_list = FiatWithdrawal::where('status',STATUS_PENDING)
                ->with(['bank.bank_form','wallet'])->orderBy('fiat_withdrawals.id', 'desc');
            ...
            return datatables()->of($withdrawal_list)
                ...
                ->addColumn('bank', function ($query) {
                    ...
                    return bankShowHtml($bank, $query?->bank?->bank_form?->title ?? "Bank");
                })
    ...
    public function fiatWithdrawRejectList(Request $request)
    {
        ...
        if ($request->ajax()) {
            $withdrawal_list = FiatWithdrawal::with("bank.bank_form")->where('status',STATUS_REJECTED)->orderBy('fiat_withdrawals.id', 'desc');
    ...
    public function fiatWithdrawActiveList(Request $request)
    {
        ...
        if ($request->ajax()) {
            $withdrawal_list = FiatWithdrawal::with("bank.bank_form")->where('status',STATUS_ACTIVE)->orderBy('fiat_withdrawals.id', 'desc');
