...
class UserService
{
    ...
    public function userKycSettingsDetails()
    {
        ...
        if ($kyc_type_is == KYC_TYPE_DISABLE) {
            $response = ['success' => false, 'message' => __('KYC is Disabled')];
    ...
    public function getUserActivities(Request $request, int $userID)
    {
        /** @var LengthAwarePaginator $response */
        $response = $this->repository->userActivities($userID, $request->limit ?? 10);

        return match(!!$response->total()){
            true  => success(__("User activities get successfully"), $response),
            false => failed(__("User activities not found"))
        };
    }
}
