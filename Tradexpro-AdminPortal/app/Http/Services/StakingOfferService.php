                        ...
                        if (isset($kycVerificationActiveList)) {
                            foreach ($kycVerificationActiveList as $item) {
                                if ($item == KYC_PHONE_VERIFICATION && !in_array($item, $userVerification)) {
                                    $response = ['success' => false, 'message' => __('KYC (Phone) is not verified'), 'data' => KYC_PHONE_VERIFICATION];
                                    return $response;
                                }
                                if ($item == KYC_EMAIL_VERIFICATION && !in_array($item, $userVerification)) {
                                    $response = ['success' => false, 'message' => __('KYC (Email) is not verified'), 'data' => KYC_EMAIL_VERIFICATION];
                                    return $response;
                                }
                                if ($item == KYC_NID_VERIFICATION && !in_array($item, $userVerification)) {
                                    $response = ['success' => false, 'message' => __('KYC (NID) is not verified'), 'data' => KYC_NID_VERIFICATION];
                                    return $response;
                                }
                                if ($item == KYC_PASSPORT_VERIFICATION && !in_array($item, $userVerification)) {
                                    $response = ['success' => false, 'message' => __('KYC (Passport) is not verified'), 'data' => KYC_PASSPORT_VERIFICATION];
                                    return $response;
                                }
                                if ($item == KYC_DRIVING_VERIFICATION && !in_array($item, $userVerification)) {
                                    $response = ['success' => false, 'message' => __('KYC (Driving) licence is not verified'), 'data' => KYC_DRIVING_VERIFICATION];
                                    return $response;
                                }
                                if ($item == KYC_VOTERS_CARD_VERIFICATION && !in_array($item, $userVerification)) {
                                    $response = ['success' => false, 'message' => __('KYC (Voter) Card is not verified'), 'data' => KYC_VOTERS_CARD_VERIFICATION];
                                    return $response;
                                }
                            }
                        }
