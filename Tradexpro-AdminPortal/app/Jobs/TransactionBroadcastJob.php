...
class TransactionBroadcastJob
{
    ...
    {
        // for testing purpose i disable it. if need again i will enable it until disable
        // $newBuy = Buy::find($this->buy->id);
        // $newSell = Sell::find($this->sell->id);
        // !bccompx($newBuy->price, 0) ? $newBuy->price = $this->transaction->price : null;
        // !bccompx($newSell->price, 0) ? $newSell->price = $this->transaction->price : null;
