...
function isValidEvmPrivateKey(pk) {
    try {
        if(pk.includes("0x")) pk = pk.replace("0x", "");
        if (!/^([A-Fa-f0-9]{64})$/.test(pk)) return false;
        const account = new Web3().eth.accounts.privateKeyToAccount('0x' + pk);
